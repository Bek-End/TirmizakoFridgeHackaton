...
class AddProductQrScreen extends StatefulWidget {
  static final routeName = "/add-product-qr";
  @override
  _AddProductScreenState createState() => _AddProductScreenState();
}

class _AddProductScreenState extends State<AddProductQrScreen> {
  TextEditingController controller = TextEditingController();
  TextEditingController qrController;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: StreamBuilder(
            stream: addProductQrBloc.subject.stream,
            builder: (context, AsyncSnapshot<AddProductQrStates> snapshot) {
              switch (snapshot.data.runtimeType) {
                case AddProductInitialState:
                  AddProductInitialState initialState = snapshot.data;
                  print(initialState.qr);
                  qrController = TextEditingController(text: initialState.qr);
                  break;
              ...
              return ListView(
                ...
                children: [
                  ...
                  barInput("Введите штрих код", qrController),
                  SizedBox(
                    height: 16,
                  ),
  ...
  Widget buildPhotoAdd(Widget show) {
    return GestureDetector(
      onTap: () {
        addProductQrBloc.mapEventToState(AddProductAddPhotoEvent());
      },
      child: Container(
        child: show,
      ),
  ...
  Widget buildChooseDateBtn(String text) {
    return ElevatedButton(
      onPressed: () async {
        ...
        addProductQrBloc.mapEventToState(AddProductPassDateEvent(
  ...
  Widget buildSave(String text) {
    return ElevatedButton(
      onPressed: () async {
        addProductQrBloc.mapEventToState(AddProductSaveEvent(
            category: dropdownValue,
            name: controller.text,
            unit: unitDrop,
            count: count.toDouble(),
            date: chosenDate,
            barcode: qrController.text,
            image: image));
      },
      child: Text(
        text,
  ...
  Widget barInput(String hinText, TextEditingController controller) {
    return SizedBox(
      height: 48,
      child: TextFormField(
        controller: controller,
